...
export const DemoTestDatepicker = (props: { value: string }) => {
...
const DatepickerComponent = (props: {
  ...
  const [calendarPosition, setCalendarPosition] =
    useState<{style: { top: number, left: number }}>({ style: { top: 0, left: 0} });
  ...
  const calendarRefCallback = useCallback((node: HTMLDivElement) => {
    if (node !== null) {
      ...
      setCalendarPosition({ style:
        {
          top: (
            (
              (inputNode.getBoundingClientRect().top
                + inputNode.getBoundingClientRect().height
                + node.getBoundingClientRect().height) < (window.innerHeight - 20)
            )
            ?
          ...
          left: (
            (
              (inputNode.getBoundingClientRect().left
                + node.getBoundingClientRect().width) < (window.innerWidth - 20)
            )
            ?
            (inputNode.getBoundingClientRect().left
  ...
  const renderInput = () => {
    ...
    return (
      <DatepickerInput type='text' maxLength={7}
        ref={inputRef}
        value={props.value}
